...
function updateContainedBy(req, callback) {
    ...
    if (req.body.quantity && !req.body.identifier) {
        updateSQL = mysql.format('UPDATE wmsinventory.ContainedBy SET quantity = ? WHERE partID = ? AND containerID = ?', [
            req.body.quantity,
            partID,
            containerID,
        ]);
    } else {
        updateSQL = mysql.format('UPDATE wmsinventory.ContainedBy SET identifier = ?, quantity = ? WHERE partID = ? AND containerID = ?', [
            req.body.identifier,
            req.body.quantity,
            partID,
            containerID,
